...
private:
	...
	typedef MagImageScalingCallback (WINAPI* MagGetImageScalingCallbackFunc)(HWND hwnd);
	...
	//HWND excluded_window_ = (HWND)0x90D88;
	...
	MagGetImageScalingCallbackFunc get_image_scaling_callback_func_;
...
static LPCTSTR kMagnifierWindowName = L"111MagnifierWindow";
...
LRESULT WINAPI MyWindowProcW(
	_In_ HWND hWnd,
	_In_ UINT Msg,
	_In_ WPARAM wParam,
	_In_ LPARAM lParam)
{
	std::cout << "MyWindowProcW" << endl;
...
bool ScreenCapturer::InitializeMagnifier() {
	...
	get_image_scaling_callback_func_ =
		reinterpret_cast<MagGetImageScalingCallbackFunc>(
			GetProcAddress(mag_lib_handle_, "MagGetImageScalingCallback"));
	...
	result = GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
		...
		reinterpret_cast<char*>(&DefWindowProcW),
		&hInstance);
	if (!result) {
...
BOOL ScreenCapturer::OnMagImageScalingCallback(
	...
	if (true) {
		...
		std::cout << "mag截图:" << exePath << "\\img_mag.bmp" << std::endl;

		char file[100] = "";
		sprintf_s(file, "img_mag.bmp");
		ofstream fout;
...
bool ScreenCapturer::CaptureImage() {
	...
	//get_image_scaling_callback_func_(magnifier_window_);
