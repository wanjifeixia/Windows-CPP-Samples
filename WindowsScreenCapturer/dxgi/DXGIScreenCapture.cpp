...
DXGIScreenCapture::DXGIScreenCapture()
    : is_initialized_(false), is_started_(false), thread_ptr_(nullptr), texture_handle_(nullptr),
      image_ptr_(nullptr), image_size_(0), key_(0) {
...
int DXGIScreenCapture::AquireFrame() {
    ...
    HRESULT hr = dxgi_output_duplication_->AcquireNextFrame(0,
                                                            &frame_info,
    ...
    d3d11_context_->CopyResource(gdi_texture_.Get(), outputTexture.Get()); //拷贝
    ...
    if (GetCursorInfo(&cursorInfo) == TRUE) {
        if (cursorInfo.flags == CURSOR_SHOWING) {
            ...
            HDC hdc;
            surface1->GetDC(FALSE, &hdc); //获取dc
            DrawIconEx(hdc,
                       cursorPosition.x - monitor_.left,
                       cursorPosition.y - monitor_.top,
                       cursorInfo.hCursor,
                       0,
                       0,
                       0,
                       0,
                       DI_NORMAL | DI_DEFAULTSIZE); //绘制
    ...
    if (!FAILED(hr)) {
        if (dsec.pData != NULL) {
            ...
            for (uint32_t y = 0; y < image_height; y++) {
                memcpy(image_ptr_.get() + y * image_width * 4,
...
//            hr = rgba_texture_->QueryInterface(__uuidof(ID3D11Resource),
//                                               (void **) &SaveTextureResource);
//            HRESULT hr = D3DX11SaveTextureToFile(d3d11_context_,
//                                                 SaveTextureResource,
//                                                 D3DX11_IFF_JPG,
//                                                 "d:\\Test0.jpg");
        }
